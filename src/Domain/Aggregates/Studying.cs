...
namespace Domain.Aggregates
{
    public class Studying : AggregateBase
    {
        ...
        private void Apply(StudyWaitingForApprovalSubmittedV1 obj)
        {
            ...
            _study = new Study(obj.StudyId, obj.Title, DateTime.Parse(obj.Metadata["$applies"]).ToUniversalTime(), null,
                obj.ResearcherId, null, StudyStatus.WaitingForApproval);
        }
        ...
        public void SubmitForApproval(SubmitStudyForApproval cmd, IStudyService studyService)
        {
            // Idempotency
            if (_study != null)
                return;
            ...
            var study = new Study(cmd.StudyId, cmd.Title, DateTime.Parse(cmd.Metadata["$applies"]), null, cmd.ResearcherId, null,
                StudyStatus.WaitingForApproval);
